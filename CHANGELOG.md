...
- The Nutrient HUD panel's height now always fits the bars currently visible. Previously a manually-resized height (`heightManual` in `nourished-ui-state.json`) was kept even when reveal-on-gain showed only one or two bars, so the whole oversized box popped up on every meal. Width is still honored as saved; `HudEditTarget.resolvedLayout` now uses the natural height.
