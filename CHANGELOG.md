...
- The dynamic UI's right-side module boxes now expose the HUD and Diet screen config options in-game, built only through MarieLib's `MarieToolbox` (`HudOptionsPanel`, `DietOptionsPanel`). The Nutrient HUD box hosts three tabs: Layout (Text Scale, Padding, Move Text and Icons — moved from the built-in rows, same UI-state store and `ScaleConfigPanel.isMoveContentEnabled` contract — plus Vertical HUD layout and HUD Corner), Behavior (Reveal HUD on nutrient gain, Hide above threshold, Show above threshold, greyed out while Hide is 1.0), and Appearance (Background opacity). A new sixth "Diet Screen" box on the Diet Screen holds Appearance (Background opacity) and Visibility (Show recent meals, Show "eat more of...", Show calorie box, Show balance box, Show inventory button). Every control reads/writes the existing `NourishedClientConfig` fields (`hudVerticalLayout`, `hudAnchor`, `hudRevealOnNutrientGain`, `hudHideAboveThreshold`, `hudShowAboveThreshold`, `hudBackgroundOpacity`, `dietBackgroundOpacity`, `showRecentMeals`, `showEatMoreOf`, `showCaloriesBox`, `showBalanceBox`, `showDietScreenButton`) with the same ranges as the Cloth screen, persisting via `NourishedClientConfig.saveNow()` — no new config keys, so existing settings carry over and hand-editing the config file still works. Sliders preview live while dragged and save once on release. The classic UI and its options are unchanged. Requires the MarieLib toolbox (next MariesLib release after 0.1.2-beta).

- The Nutrient HUD, Calorie History and Activity Log panels now have properly rounded corners (radius 4, `HudDrawHelpers.PANEL_CORNER_RADIUS`) instead of the old one-pixel notch, via MarieLib's new `RenderContext.drawRoundedRect` overload with a radius. The classic HUD and the Diet screen are unchanged. Requires the next MariesLib release after 0.1.2-beta.

- Nutrient HUD: new "Icon and text brightness" slider in the Appearance tab of its dynamic-UI box (20%-100%, default 100% = unchanged), bound to the new client config key `hudContentBrightness` (dynamic UI only — no Cloth Config entry, and the classic HUD ignores it). It scales the RGB of the row labels and percentage text and tints the nutrient icons (via the GUI shader color around the item draw, restored in a `finally`); bars, backgrounds and the vertical layout's missing icons are unaffected. Previews live while dragging and saves on release.

- The same "Icon and text brightness" slider (20%-100%, default 100%) is now on every dynamic-UI module box: the Calorie History and Activity Log HUD boxes and the new Diet Screen box (Appearance tab, next to Background opacity), bound to new client config keys `calorieHudContentBrightness`, `activityLogHudContentBrightness` and `dietContentBrightness` (dynamic UI only — no Cloth Config entry, classic screens ignore them). These panels draw through a small new `BrightnessRenderContext` wrapper that dims every `drawText` color and tints every `drawItem` icon while passing fills/bars/borders/clips through, and returns the original context untouched at 100%. The Calorie History and Activity Log boxes now host a toolbox panel (Layout: Text Scale, Padding, Move Text and Icons, over the same UI-state keys as their previous built-in rows; Appearance: brightness) instead of the built-in rows. The Diet Screen's five sub-box entries are unchanged; the Diet Screen brightness applies to the whole dynamic Diet panel including all five boxes.

- Module box cleanup: the "Move Text and Icons" toggle and the Nutrient HUD's "HUD Corner" cycle are removed from the dynamic-UI boxes (the boxes are dragged into place, so neither is needed; `hudAnchor` itself is unchanged). Because removing the toggle would leave a previously-enabled content-move mode stuck on, opening a module box now switches its `#moveContent` flag off. The Calorie History and Activity Log boxes now carry the same options as the Nutrient HUD box that apply to them: Layout (Text Scale, Padding) and Appearance (Background opacity over the existing `calorieHudBackgroundOpacity`/`activityLogHudBackgroundOpacity`, plus Icon and text brightness); the Nutrient HUD's vertical layout, reveal-on-gain and threshold options are about nutrient bars and don't apply to them. The classic Nutrient HUD renderer (`HudDrawHelpers.renderIcon`/`drawScaledLabel`) now also honors `hudContentBrightness`, so the slider takes effect with `hudClassicMode` on as well as off.

- The Icon and text brightness sliders now go past 100%: range 20%-200% (was 20%-100%) for `hudContentBrightness`, `calorieHudContentBrightness`, `activityLogHudContentBrightness` and `dietContentBrightness`. Above 100% each text color channel is scaled up and capped at 255, so colors wash toward white, and icons are tinted with a shader color above 1.0. 100% remains unchanged; saved values are unaffected.
