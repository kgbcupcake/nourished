...
- Every Intake Breakdown row's "Text size" slider resized its icon too — MariesLib's shared `BarRowComponent` had an icon-scale/text-scale entanglement bug; see MariesLib's changelog for the underlying fix (this mod needed no changes, just the updated dependency).
- The Diet Screen's own panel (the outer window, not a sub-box) couldn't be resized narrower than its full natural width via the corner-resize handle — its width floor was pinned to 100% of natural while its height already floored much lower (down to the minimize threshold), so shrinking only ever worked vertically. Width now floors at the same 60% shrink multiplier every individual sub-box's own resize already uses; a narrower panel just clips its fixed-size right-column content, the same way a shorter one already clips its bottom content.
- Recent Meals' "Hide Bars" toggle (Behavior tab) did nothing — each row's name (this box's stand-in for a bar, per its own "Move Bars" wiring) is drawn through a brightness-only context rather than a real `context.drawBar` call, so MariesLib's automatic Hide Bars enforcement (which only intercepts `drawBar`/`drawVerticalBar`) never saw it. The row name draw (and its extent report) is now skipped explicitly when `MarieModuleSettings.isBarsHidden` is on, the same manual-check pattern the box's own "Hide Icons" glyph-stripping already used.
- Recent Meals' Behavior tab showed a "Move Text" toggle that did nothing: unlike Calories/Balance/Eat More (whose header has its own offset and whose remaining body text moves under "Move Text"), Recent Meals has no body text left after its header — the row names move under "Move Bars" instead — so nothing ever consumed the toggle. Its options panel now leaves "Move Text" out entirely, via new MariesLib `StandardPanelBuilder#withoutMoveText`/`DietOptionsPanel#forModule` overload — see MariesLib's changelog for the `withoutMoveText` half.
- Every Intake Breakdown row's options panel had no Colors tab at all — `DietOptionsPanel#forIntakeBar` never passed a colors consumer through to `forModule`, so the `StandardPanelBuilder#extraTabs` call that builds the tab was never made, unlike every other Diet box (Calories, Balance, Recent Meals, Eat More, Active Effects all pass one). Each row now gets its own Colors tab exposing its label text, bar track and border (`IntakeBarComponent` previously drew all three with the shared `TEXT`/`DIET_BAR_TRACK`/`BORDER` roles directly, unlike every other box's own dedicated colors) — and, since Intake Breakdown has five structurally-identical rows sharing one panel definition rather than one bespoke box per nutrient (see `#forIntakeBar`'s doc), each of these three is a *per-nutrient* color (new `NourishedColors#intakeBarTextKey`/`intakeBarTrackKey`/`intakeBarBorderKey`, dynamically registered per registered nutrient by new `NourishedColors#registerIntakeBarColors`), so Fruits/Vegetables/Proteins/Grains/Dairy are independently colorable instead of one setting applying to all five. The bar's fill, percent text and trend arrow stay driven by the existing global per-nutrient color system (the main Diet colors panel's "Nutrients" tab) rather than duplicated again here.
- The Nourished guide book couldn't be crafted — a leftover Patchouli-era recipe (`nourished_guide.json`) shared the exact same book+wheat ingredients as the real Modonomicon recipe but still pointed at `patchouli:guide_book`, an item that no longer exists now that Patchouli isn't a dependency, and the two conflicted. The stale recipe is removed, and the `obtained_guide` advancement now checks for the real `nourished:nourished_book` item instead of the old `modonomicon:modonomicon` + `book_id` component pairing.
...
- Icon size slider now works on Calories, Balance, Recent Meals, and Eat More. Active Effects (text only) shows a single Size slider. Recent Meals' size slider, previously mislabeled "Header size", is now "Text size".
- Every Diet sub-box's options panel (Calories, Balance, Recent Meals, Eat More Of, Active Effects, and each Intake Breakdown row) drops the Layout tab's Padding slider: every box is now directly drag-resizable and every piece of its content individually movable (Move Text/Header/Icons/Bars), so a separate padding-percentage knob was redundant with those. Via new MariesLib `StandardPanelBuilder#withoutPadding`, now applied unconditionally in `DietOptionsPanel#forModule`.
- Eat More Of's Behavior tab no longer shows a "Hide Text" toggle — it has no text separate from the box's one suggestion line, which "Hide Window" already covers, so the toggle had nothing distinct left to hide. Via new MariesLib `StandardPanelBuilder#withoutHideText` — see MariesLib's changelog for that half.
- Eat More Of's Behavior tab no longer shows a "Move Text" toggle either — same reasoning as the "Hide Text" removal above: it has no body text at all separate from its header (its body is just the suggested-food icons), so the toggle had nothing of its own to move.
- The hub editor window now has its own "Editor" sidebar entry, for customizing the hub's own chrome colors (background, border, title text, accent) independent of any box it edits — 4 new color keys (`NourishedColors#HUB_BACKGROUND`/`HUB_BORDER`/`HUB_TITLE`/`HUB_ACCENT`) feeding MariesLib's new per-instance `HubPanel` color overrides (see MariesLib's changelog).
- Every Diet Screen box's options panel (and every other module's, across the whole mod) now shows one "Reset This Module" button at the top of its Layout tab instead of three separate, narrower ones (Layout's and Style's own "Reset This Tab", Behavior's "Reset Positions") — it resets everything those three did, plus the box's own drag/resize position and size and its colors, in one click. Also removed the now-redundant manual tab reset from the Diet Screen's own whole-screen options panel. Via MariesLib's new `StandardPanelBuilder#build`/`MarieToolbox.PanelBuilder#resetEverything` — see MariesLib's changelog for the underlying mechanism. Separately, the Diet Screen's hub editor window can now be resized much smaller (its sidebar and a selected group's row list each scroll independently once they no longer all fit) — see MariesLib's changelog for the `HubPanel` half.

### Added

- Calories, Balance, Recent Meals, Eat More and Active Effects each get an independent "Header size" slider (Style tab) and "Hide Header" toggle (Behavior tab) for their title, separate from whatever else they draw — via new MariesLib `StandardPanelBuilder#withHeaderSize`/`#headerSizeLabel`/`#withHideHeader` and `MarieModuleSettings#headerScale`/`#isHeaderHidden` (see MariesLib's changelog). Recent Meals' Behavior tab previously showed a "Hide Text" toggle that did nothing — its title is drawn through a brightness-only context that MariesLib's automatic Hide Text enforcement never sees, the same class of bug already fixed for its "Hide Bars" toggle — it's now "Hide Header" and actually hides the title. Recent Meals' and Eat More's "Text size" sliders (the title was the only thing either ever drove) are dropped outright in favor of Header size, via new MariesLib `StandardPanelBuilder#withoutTextSize`. Active Effects' title no longer draws at a fixed 0.9x of its effect-line text; its remaining Style-tab slider, now driving only the effect lines, is relabeled "Bars". Calories' own "Text size" slider (its calorie value is the only thing it drives) is relabeled "Number size".
- Icon size on Calories, Balance, Recent Meals and Eat More no longer visibly tracks Text/Number size while dragging it. Two separate bugs stacked here: (1) until a box's own Icon size slider had been touched once, its *stored* value quietly followed Text size (a MariesLib default meant to keep pre-icon-size-split modules looking unchanged) — fixed via new MariesLib `StandardPanelBuilder#independentIconSize`. (2) even after that fix, the actual *drawn* icon still silently re-scaled: MariesLib's shared display-settings wrapper (`MarieModuleSettings#withDisplaySettings`) has always applied its own `iconScale ÷ textScale` ratio on top of whatever scale a module's renderer already passes to `drawItem`, for modules that rely on the wrapper alone to turn a single text-based scale into their icon size — these four boxes already resolve their own independent icon scale before calling `drawItem`, so that extra ratio was double-applying against Number size behind the scenes, invisible to the stored/persisted values (which is why they checked out correct) but very visible on screen. Fixed via new MariesLib `MarieModuleSettings#withDisplaySettings(..., boolean iconFollowsText)`, called with `false` so the wrapper passes the already-resolved icon scale straight through (see MariesLib's changelog for both halves).
- Recent Meals' Header size slider no longer pushes the meal rows further down the bigger it gets — a regression from adding Header size itself: the header-to-first-row gap was (re)computed from Header size's ratio to the box's fit scale, the same "keep zoomed text from colliding with the next line" compensation the box used back when one slider drove both the header and, briefly, its own spacing. Header size now affects only the header's own draw size, like Calories'/Balance's header already did (their header-to-body distance was always a fixed constant, never derived from any slider).
...
- The Diet Screen's editor gear-icon list (Calories/Balance/Recent Meals/Eat More/Active Effects/Legend/Diet Screen) is now a single resizable/draggable hub window with a sidebar and a content pane, instead of a stack of thin collapsed tabs anchored to a screen corner — built on MariesLib's new `HubPanel`. It's still an overlay drawn on top of the Diet Screen (not a full-screen takeover), so the boxes stay visible and draggable behind it exactly as before. The Intake Breakdown's bar rows, which previously had nowhere in this list to be edited from at all, now live under one "Intake" entry: selecting it shows the current nutrient rows (read fresh from the nutrient registry and the live bar order every time, so it stays correct if more nutrients are ever added), and clicking a row opens its full settings (icon, label, bar, colors, border) in a small popup, the same interaction as the color picker.
- Each Intake Breakdown row's Behavior tab now has a "Move Icon" toggle, independent of the existing "Move Icons" — the icon can now be dragged within its own small box without moving the box itself, while "Move Icons" still moves the box (the icon riding along with it, same as before). Via new MariesLib `StandardPanelBuilder#withIconInnerMove` (see MariesLib's changelog).
...
- Clicking a color swatch anywhere in the Diet Screen's new hub editor did nothing — no color picker opened. The hub never wired up the click-to-picker listener `ScaleConfigPanel` used to set up itself, and separately, the Intake group's row list was rebuilt as brand-new options-panel instances on every single render call, which would have thrown away that listener (and any of the panel's own UI state) the instant the next frame replaced it. Fixed on both sides (MariesLib's `HubPanel` now wires the listener itself; `intakeGroupEntry()` here now caches each row's options panel by slot id instead of rebuilding it every frame) — see MariesLib's changelog for the `HubPanel` half.
- Resizing an Intake Breakdown row, the header or the legend from the bottom-left corner handle pushed its icon/label/bar away from its own row card, leaving an empty gap where they used to sit. `DietScreenEditTarget`'s left/top-edge content-anchor compensation (`keepContentInPlace`, written for the left column's boxes, whose content is positioned through a separate persisted local-coordinate layout that a resize's origin shift really does need compensating for) was also being applied to these three — but their content is drawn directly off their own live `Bounds` at a fixed scale with no such separate layout to compensate for, so the compensation just shoved it away from the box that was, correctly, tracking the resize on its own. No longer called for the header, bars, or legend.
- The Intake Breakdown header's and legend's Text size slider did nothing, and hiding either of them (Behavior tab's "Hide Window") left them drawn exactly as before — MariesLib's `TitleBarComponent`/`LegendComponent` weren't wired up for either. Also removed the legend's colored swatch squares in favor of coloring the "Good"/"Low"/"Critical" label and its range line directly with that entry's own accent — see MariesLib's changelog for both.
