...
- Icon size on Calories, Balance, Recent Meals and Eat More no longer ever follows Text size. Previously, until a box's Icon size slider had been explicitly touched once, its icon quietly tracked whatever Text size was set to (a MariesLib default meant to keep pre-icon-size-split modules looking unchanged) — dropping Recent Meals' and Eat More's Text size row above would otherwise have left their icon permanently reading a leftover Text size value the player can no longer see or change. Icon size on these four boxes now always starts at a plain 100% and only ever moves when its own slider does, via new MariesLib `StandardPanelBuilder#independentIconSize`/`MarieModuleSettings#iconScale(PersistenceProvider, String, boolean)` (see MariesLib's changelog).
